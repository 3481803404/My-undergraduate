...
private:
    ...
    string signtable[100];
    int tempCount=0;
    int signcnt=0;
    void addsigntable(const string& str){
        if(str.empty())return;
        for(int i=0;i<signcnt;i++)
            if(str==signtable[i])
                return;
        signtable[signcnt++]=str;
    }
    void lex() {
        size_t pos = 0;
    ...
    string generateTempVariable() { //产生中间变量
        string str = "T" + to_string(tempCount++);
        addsigntable(str);
        return str;
    }
